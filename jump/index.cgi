...
use lib "/usr/local/perl-modules/Ylink";
use Ylink qw/:all/;
...
sub main
{
	if (defined(param('url'))) {
		...
		if ($iret == 0) {
			...
			printPage('NU link shortener: Success', 
...
sub displayMainPage
{
	...
	if (defined($errormess)) {
		...
		printPage($title,
		          div({id=>'content'}, p({-id=>'title'}, 'Northwestern folks can create a shorter link that anyone can use.') .
		          div({id=>'form'},
		          start_form(-name=>'ylink', -id=>'ylinkform', -action=>$SECUREURL . 'index.cgi', -method=>'GET') .
		         			 $formattederror .
		               p('Enter URL: ' .
